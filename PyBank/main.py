#Import modules
import os, csv
from pathlib import Path
#Make empty lists 
total_months = []
total_profit = []
monthly_profit_change = []
#Open csv file 
csv_file_path = Path('Resources/budget_data.csv')
with open(csv_file_path, newline='', encoding='utf-8') as budget_data:
    csvreader = csv.reader(budget_data, delimiter=",")
    #Bypass the header
    header = next(csvreader)
    #rows
    for row in csvreader:
        #Append total months and total profit/losses
        total_months.append(row[0])
        total_profit.append(int(row[1]))
    #loop through profits to get monthly change
    for i in range(len(total_profit)-1):
        #Find difference between 2 months 
        monthly_profit_change.append(total_profit[i+1]-total_profit[i])
#Calc the min/max of monthly profit change to find greatest increase/greatest decrease in profits
max_increase = max(monthly_profit_change)
max_decrease = min(monthly_profit_change)
#Pair these values with their respective months
max_increase_month = monthly_profit_change.index(max(monthly_profit_change))
max_decrease_month = monthly_profit_change.index(min(monthly_profit_change))
#Print statements for terminal
print("Financial Analysis")
print("----------------------------")
print(f"Total Months: {len(total_months)}")
print(f"Total: ${sum(total_profit)}")
print(f"Average Change: ${round(sum(monthly_profit_change)/len(monthly_profit_change),2)}")
print(f"Greatest Increase in Profits: {total_months[max_increase_month]} (${(str(max_increase))})")
print(f"Greatest Decrease in Profits: {total_months[max_decrease_month]} (${(str(max_decrease))})")
#Output file
output_file = Path('Resources/Financial_Analysis_Summary.txt')
with open(output_file, "w") as file:
    #Printing in Financial_Analysis_Summary text file
    file.write("Financial Analysis")
    file.write("\n")
    file.write("----------------------------")
    file.write("\n")
    file.write(f"Total Months: {len(total_months)}")
    file.write("\n")
    file.write(f"Total: ${sum(total_profit)}")
    file.write("\n")
    file.write(f"Average Change: ${round(sum(monthly_profit_change)/len(monthly_profit_change),2)}")
    file.write("\n")
    file.write(f"Greatest Increase in Profits: {total_months[max_increase_month]} (${(str(max_increase))})")
    file.write("\n")
    file.write(f"Greatest Decrease in Profits: {total_months[max_decrease_month]} (${(str(max_decrease))})")